...
以下为规范流程中的阶段定义，用于约束 Agent 行为、获得稳定输出；在各阶段下挂载角色、活动与 skill。执行可为单 Agent 连续推进或多 Agent 协作，阶段与角色/skill 的映射需在最终结构化产出中可被 Agent 解析并用于自动启用相应 skill。

| 阶段 | 英文标识 | 主要产出 | 说明 |
|------|----------|----------|------|
| 项目启动 | initiation | 项目计划、流程裁剪说明、阶段与活动清单、可选项目章程摘要 | 根据用户问题/需求按活动规范对项目活动裁剪，确定项目流程与计划；依据 PMBOK 裁剪与 ISO/IEC/IEEE 16326 项目管理计划 |
| 需求 | requirements | 用户故事、PRD、验收标准、路线图/Backlog | 需求 elicitation、分析与优先级；产出可供设计与开发使用的需求基线 |
| 需求评审 | requirements-review | 评审结论、待澄清/须补充项 | 对需求/用户故事/PRD 的完整性、一致性、可测性评审 |
| 架构/设计 | design | 架构设计、API 设计、数据库设计、ADR | 技术选型、模块划分、接口与数据模型 |
| 设计/架构评审 | design-review | 设计评审结论、风险与改进建议 | 对架构与设计文档、安全设计、DBA/DevOps 方案等的评审 |
| 开发 | development | 代码、PR、文档草稿 | 前端/后端/全栈实现；可含 DBA 的 schema/迁移、安全的实现与配置 |
| 代码评审 | code-review | 评审结论（P0–P3 等）、修改建议 | 对 PR/代码变更的规范、安全、性能、可维护性评审 |
| 测试 | testing | 测试计划/用例、测试结果、质量报告 | 测试策略、自动化与手工测试、质量门禁 |
| 测试评审 | test-review | 测试评审结论、覆盖度与用例改进建议 | 对测试计划/用例的覆盖度与可执行性评审 |
| 部署/发布 | deployment | 流水线、发布包、发布说明 | CI/CD、环境与发布策略、回滚预案 |
| 运维/复盘 | operations | 监控与告警、故障复盘、Runbook、SLO 报告 | 可靠性、容量与成本、事件响应与改进 |
