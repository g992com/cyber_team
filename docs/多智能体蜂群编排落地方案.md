...
- 解析并加载规范：
  ...
  - 读取 `process/process.md`：理解每个阶段的目标与完成条件；
  ...
  - 读取 `state.yaml`：获取当前阶段、已完成阶段等状态信息。
- 处理用户输入：
  - 接收用户的自然语言需求/问题；
...
- 状态与产出更新：
  - 根据阶段完成条件更新 `state.yaml` 中的 `current_phase`、`completed_phases`；
  - 视需要更新业务项目中的 `project-docs-index.yaml`（例如记录文档路径、状态、更新时间等）。
...
- **处理任务并产出结果**：
  ...
  - 读取业务项目的 `project-docs-index.yaml`，定位/创建对应阶段产出物；
...
```json
{
  ...
  "inputs": {
    ...
    "project_docs_index_path": "project-docs-index.yaml",
...
- `artifacts`：与 `project-docs-index.yaml` 与 frontmatter 约定保持一致；
...
1. **规范层面的完成条件**：
   - 写在 `process/phases.yaml` 或 `process/process.md` 中，例如：
...
推进后，Orchestrator 可选择将新的 `state.yaml` 写回仓库，或通过外部状态存储（数据库、配置中心等）进行持久化。
...
- 所有阶段产出物路径，均以业务项目根目录下的 `project-docs-index.yaml` 为“单一事实源”；
...
- 如需新建文档，应在写入后协助更新 `project-docs-index.yaml`（由 Role Agent 自己或通过 Orchestrator 完成）。
...
1. **项目启动**：
   ...
   - Orchestrator 初始化 `state.yaml`，将 `current_phase` 设为 `initiation`；
...
- 与业务项目中文档索引与元数据协议（`project-docs-index.yaml` + frontmatter）保持完全一致。
