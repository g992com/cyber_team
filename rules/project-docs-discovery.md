...
- **索引文件路径**：项目仓库根目录下的 `project-docs-index.yaml`（若项目约定放在其他路径，以项目约定为准）。
- **何时读取**：在需要发现或引用某阶段的产出物（如 PRD、用户故事、架构文档、测试计划、发布说明等）时，**先读取** `project-docs-index.yaml`，根据阶段 id 或任务目标找到对应条目，再按条目中的相对路径读取具体文档。
...
- 若项目中有 `state.yaml` 且包含 `current_phase`，可优先查看当前阶段对应的索引区块，再按需查看其他阶段。

## 使用流程

1. 读取 `project-docs-index.yaml`，确定目标阶段与产出物类型对应的文件路径。
...
- 索引文件可从本规范仓库的 `process/project-docs/project-docs-index.yaml` 复制到业务项目后，按实际产出填写或新增路径。
