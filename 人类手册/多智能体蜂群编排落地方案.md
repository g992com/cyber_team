...
- **背景**：本仓库采用**单仓模式**，规范位于业务项目内的 `.cyber_team/`。软件开发活动抽象为：
  - `.cyber_team/process/`：阶段与流程；
  - `.cyber_team/roles/`：角色与职责；
  - `.cyber_team/mapping/phase-role-skill.yaml`：阶段-角色-skill 映射；
  - `.cyber_team/skills/manifest.yaml` + 各 `.cyber_team/skills/*/SKILL.md`：技能行为与 SOP；
- `state.yaml`：当前阶段与进展快照（**业务项目仓库内**）；`.cyber_team/process/state.yaml` 仅作 schema 示例/模板。
  - 以及 `.cyber_team/process/project-docs/project-docs-index.yaml` 与 `.cyber_team/process/artifact-metadata-convention.md` 对产出物的结构化约定。
- **与规范库关系**：单仓下业务项目自带 `.cyber_team` 快照，Orchestrator 与 Role Agent 直接读取业务项目内的 `.cyber_team/` 即可，无需多根工作区。
...
- 解析并加载规范：
  - 读取 `.cyber_team/process/phases.yaml`：获得阶段列表与顺序；
  - 读取 `人类手册/process/process.md`：理解每个阶段的目标与完成条件；
  - 读取 `.cyber_team/roles/roles.yaml`：获得角色池及其参与阶段；
  - 读取 `.cyber_team/mapping/phase-role-skill.yaml`：确定阶段-角色-skill 映射；
  - 读取 `.cyber_team/skills/manifest.yaml`：查找每个 skill 对应的 source 与说明；
...
每个 Role Agent 与 `.cyber_team/roles/roles.yaml` 中的一个或一组角色对应，例如：
...
- **理解自身职责与 SOP**：
  - 启动时读取 `.cyber_team/roles/roles.yaml` 中自身角色定义；
  - 读取 `人类手册/roles/*.sop.md` 中的对应 SOP（如有）。
- **加载与执行技能**：
  - 根据 Orchestrator 下发的 `skill_name`，在 `.cyber_team/skills/manifest.yaml` 中查找其 source；
...
- **处理任务并产出结果**：
  ...
  - 根据 `.cyber_team/process/artifact-metadata-convention.md` 写入/更新文档及 frontmatter；
...
- `phase_id`：阶段 id，与 `.cyber_team/process/phases.yaml`、`state.yaml` 对齐；
- `role_id`：角色 id，与 `.cyber_team/roles/roles.yaml` 对齐；
- `skill_name`：将在 `.cyber_team/mapping/phase-role-skill.yaml` 与 `.cyber_team/skills/manifest.yaml` 中出现；
...
Orchestrator 启动时，应将 `.cyber_team/process/phases.yaml` 解析为一个有序状态机，例如：
...
在某个 `current_phase` 下，Orchestrator 通过 `.cyber_team/mapping/phase-role-skill.yaml` 查找需要激活的 `(role_id, skill_name)` 列表，例如：
...
1. **规范层面的完成条件**：
   - 写在 `.cyber_team/process/phases.yaml` 或 `人类手册/process/process.md` 中，例如：
...
- 所有产出物文件顶部使用 YAML frontmatter，字段遵循 `.cyber_team/process/artifact-metadata-convention.md`；
...
1. **Phase 1：只自动化 PRD 阶段**
   ...
   - 使用已有的 `.cyber_team/skills/prd-requirements`、`.cyber_team/skills/prd-review` 作为 SOP 核心。
2. **Phase 2：加入架构与测试设计**
...
- Orchestrator 对 `.cyber_team/process/`、`.cyber_team/roles/`、`.cyber_team/mapping/`、`.cyber_team/skills/`、`state.yaml` 的解析是稳定的；
